%% To do: Pick a lower degree function
...
%\clearpage
\begin{tabbing}
2. \textbf{EXAMPLE.} \= Use the definition of difference quotient to find the slope of  \\ \> the \textbf{secant} line through $y=f(x)=15+\frac58x^4$ between $x$ and $x+h$. \\[0.5ex]
\> \emph{Hint:} 
%$f(x+h) = 5x^4/8+20x^3h/8+30x^2h^2/8+20xh^3/8+5h^4/8+15$
$\phantom( f(x+h) \phantom{{}-f(x))\frac1h} = \phantom( 15 + \frac58x^4+\frac{20}8x^3h+\frac{30}8x^2h^2+\frac{20}8xh^3+\frac58h^4$
\\[3ex] 
3. \textbf{Exercise.} \> Use the definition of derivative and your answer to \textbf{Example 2} to find the slope of \\ \>  the \textbf{tangent} line to $y=f(x)=15+\frac58x^4$ at $x$. \\
\diagram{node [right, yshift=-2mm] {$y=f(x)=15+\frac58x^4$}}{
	% tick marks and coordinates for closed circles
	\foreach \x in {0, ..., 4}{
		\draw (\x, {(\x-1)^4*5/8/100+15/100}) coordinate (\x);
...
4. \textbf{Exercise.} \> Use the \textbf{diagram} to estimate the slope of \\\> the \textbf{secant} through $y=15+\frac58x^4$ \textbf{between 2 and 4}. \\
5. \textbf{Exercise.} \> Use the \textbf{diagram} to estimate the slope of \\ \> the \textbf{tangent} line to $y=f(x)=15+\frac58x^4$ \textbf{at 2}. \\
6. \textbf{Exercise.} \> Use your answer to \textbf{Exercise 3} to confirm your answer to \textbf{Exercise 5}.
\end{tabbing}
\end{minipage}

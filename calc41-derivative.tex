...
\usepackage{tikz, amsmath, mathtools, multicol}
...
\newcommand\stacktwo[2]{\hfill\(\begin{matrix}\text{#1}\\[-0.7ex]\text{#2}\end{matrix}\)\hfill}
\newcommand\stackthree[3]{\hfill\(\begin{matrix}\text{#1}\\[-0.7ex]\text{#2}\\[-0.7ex]\text{#3}\end{matrix}\)\hfill}
\newcommand\stackfour[4]{\hfill\(\begin{matrix}\text{#1}\\[-0.7ex]\text{#2}\\[-0.7ex]\text{#3}\\[-0.7ex]\text{#4}\end{matrix}\)\hfill}
...
\vfill
\stackfour{average}{velocity}{over}{$[x, x{+}h]$}
= \stackfour{avg rate}{of change}{over}{$[x, x{+}h]$}
= \stackfour{slope of}{secant}{btw}{$x, x{+}h$}
= \hfill$\frac{\Delta y}{\Delta x}$\hfill 
= \hfill$\frac{y_2 - y_1}{x_2 - x_1}$\hfill 
= \hfill{\LARGE\boxed{\phantom{\frac{f(x+h)-f(x)}h}}}\hfill 
= \stackthree{definition of}{difference}{quotient}

\vfill
\stacktwo{velocity}{at $x$}
= \stackthree{rate of}{change}{at $x$}
= \stackthree{slope of}{tangent}{at $x$}

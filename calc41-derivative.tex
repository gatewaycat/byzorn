...
\begin{tikzpicture}[very thick, scale=2]
	% grid
	\draw[gray, semithick, dotted, step=0.25] (0,0) grid (5.5,2.75);
	\draw[gray, thick] (0,0) grid (5.5,2.75);
	% axes
	\draw[<->] (0,2.75) node[below left] {distance [mi]} to (0,0) to (5.5,0) node[right] {time [hr]};
...
%	\clip (0,0) rectangle (8.5,2.5);
	...
	% secant
	\draw[red, domain=2.5:5.5] 
		plot (\x, {0.75*(\x-3)+0.25})
		node [right] {\,\,\large secant};
	% tangent
	\draw[red, domain=1.5:5.5] plot (\x, {0.2*(\x-3)+0.25})
		node [right] {\,\,\large tangent};
	% closed circles
	\foreach \x/\n in {3/x, 5/x+h}{
		\draw[black, fill=black] (\x) circle (2pt);
		}
	% fillin (x1, y1) = (x, f(x))

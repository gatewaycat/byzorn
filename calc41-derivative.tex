...
\vspace{-2ex}
\hfil
\diagram{y=f(x)\phantom{{}=15+\frac58x^4}}{
...
%\clearpage
\begin{tabbing}
2. \textbf{Example.} \= Use the definition of difference quotient to find the slope of  \\ \> the \textbf{secant} line through $y=f(x)=15+\frac58x^4$ between $x$ and $x+h$. \\[1ex] \> \emph{Hint:} 
...
\\[1ex] 
3. \textbf{Exercise.} \> Use the definition of derivative to find the slope of \\ \>  the \textbf{tangent} line to $y=f(x)=15+\frac58x^4$ at $x$. \\[-2.5ex]
\diagram{y=f(x)=15+\frac58x^4}{
	% tick marks and coordinates for closed circles
	\foreach \x in {0, ..., 4}{
		\draw (\x, {(\x-1)^4*5/8/100+15/100}) coordinate (\x);
...
4. \textbf{Exercise.} \> Use the \textbf{diagram} to estimate the slope of  \\ \> the \textbf{secant} line to $y=f(x)=15+\frac58x^4$ \textbf{between 2 and 4}. \\[1ex]
5. \textbf{Exercise.} \> Use your answer to Exercise 3 above to estimate the slope of  \\ \> the \textbf{tangent} line to $y=f(x)=15+\frac58x^4$ \textbf{at 2}.
\end{tabbing}
\end{minipage}
